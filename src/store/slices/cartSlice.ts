...
//   initialState,
//   reducers: {
// addToCart: (state, action: PayloadAction<{ item: Item; size: ItemSize; quantity: number }>) => {
//       const { item, size, quantity } = action.payload;
//       const existingItem = state.items.find(
//         i => i.id === item.id && i.type === item.type && i.selectedSize === size
//       );
//       if (existingItem) {
//         existingItem.quantity += quantity;
//       } else {
//         state.items.push({ ...item, quantity, selected: true, selectedSize: size });
//       }
//       state.totalPrice = calculateTotal(state.items);
//     },
//     toggleItemSelection: (state, action: PayloadAction<{ id: number; type: string,size:ItemSize }>) => {
...
//       }
//     },
//     removeFromCart: (state, action: PayloadAction<{ id: number, type: string,size:ItemSize }>) => {
//       const existingItem = state.items.find(
//         item => item.id === action.payload.id && item.type === action.payload.type
//       );
...
// Helper to calculate total based on selected items and their specific size prices
const calculateTotal = (items: CartItem[]) => {
  return items
    .filter(item => item.selected)
    .reduce((total, item) => {
